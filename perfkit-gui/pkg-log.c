...
pkg_log_level_str (GLogLevelFlags log_level) /* IN */
{
 ...
 */
static void
pkg_log_write_to_channel (GIOChannel  *channel, /* IN */
 ...
 */
static void
pkg_log_handler (const gchar    *log_domain, /* IN */
                 ...
                 gpointer        user_data)  /* IN */
{
	...
	if (G_LIKELY(channels->len)) {
		level = pkg_log_level_str(log_level);
		...
		buffer = g_strdup_printf("%s.%04ld  %s: %12s[%d]: %8s: %s\n",
		                         ...
		                         hostname, log_domain,
		                         pkg_log_get_thread(), level, message);
		G_LOCK(channels_lock);
		g_ptr_array_foreach(channels, (GFunc)pkg_log_write_to_channel, buffer);
 ...
 */
void
pkg_log_init (gboolean     stdout_,  /* IN */
...
#error "Target platform not supported"
		...
		g_log_set_default_handler(pkg_log_handler, NULL);
