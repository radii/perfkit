...
/**
 * pk_shell_subscription_get_buffer_cb:
 * @object: A #PkConnection.
 * @result: A #GAsyncResult.
 * @user_data: A #gpointer.
 *
 * Asynchronous completion of pk_connection_subscription_get_buffer_async().
 *
 * Returns: None.
 * Side effects: Blocking AsyncTask is signaled.
 */
static void
pk_shell_subscription_get_buffer_cb (GObject       *object,    /* IN */
            GAsyncResult  *result,    /* IN */
            gpointer       user_data) /* IN */
{
	AsyncTask *task = user_data;
	...
	ENTRY;
	task->result = pk_connection_subscription_get_buffer_finish(
			PK_CONNECTION(object),
			result,
			task->params[0], /* timeout */
			task->params[1], /* size */
			&task->error);
	async_task_signal(task);
	EXIT;
...
/**
 * pk_shell_subscription_get_buffer:
 * @line: An #EggLine.
 * @argc: The number of arguments in @argv.
 * @argv: The arguments to the command.
 * @error: A location for #GError, or %NULL.
 ...
 */
static EggLineStatus
pk_shell_subscription_get_buffer (EggLine  *line,   /* IN */
                                  gint      argc,   /* IN */
                                  gchar    *argv[], /* IN */
                                  GError  **error)  /* OUT */
{
	...
	task.params[0] = &timeout;
	task.params[1] = &size;
	pk_connection_subscription_get_buffer_async(conn,
	                             subscription,
	                             NULL,
	                             pk_shell_subscription_get_buffer_cb,
	                             &task);
	...
	tmp = g_strdup_printf("%d", (gint)timeout);
	egg_line_set_variable(line, "1", tmp);
	g_free(tmp);
	g_print("%16s: %d\n", "size", (gint)size);
	tmp = g_strdup_printf("%d", (gint)size);
	egg_line_set_variable(line, "2", tmp);
	g_free(tmp);
	RETURN(EGG_LINE_STATUS_OK);
}
...
/**
 * pk_shell_subscription_get_sources_cb:
 * @object: A #PkConnection.
 * @result: A #GAsyncResult.
 * @user_data: A #gpointer.
 *
 * Asynchronous completion of pk_connection_subscription_get_sources_async().
 *
 * Returns: None.
 * Side effects: Blocking AsyncTask is signaled.
 */
static void
pk_shell_subscription_get_sources_cb (GObject       *object,    /* IN */
            GAsyncResult  *result,    /* IN */
            gpointer       user_data) /* IN */
{
	AsyncTask *task = user_data;
	...
	ENTRY;
	task->result = pk_connection_subscription_get_sources_finish(
			PK_CONNECTION(object),
			result,
			task->params[0], /* sources */
			task->params[1], /* sources_len */
...
/**
 * pk_shell_subscription_get_sources:
 * @line: An #EggLine.
 * @argc: The number of arguments in @argv.
 * @argv: The arguments to the command.
 * @error: A location for #GError, or %NULL.
 ...
 */
static EggLineStatus
pk_shell_subscription_get_sources (EggLine  *line,   /* IN */
                                   gint      argc,   /* IN */
                                   gchar    *argv[], /* IN */
                                   GError  **error)  /* OUT */
{
	...
	task.params[0] = &sources;
	task.params[1] = &sources_len;
	pk_connection_subscription_get_sources_async(conn,
	                             subscription,
	                             NULL,
	                             pk_shell_subscription_get_sources_cb,
	                             &task);
...
static EggLineCommand subscription_commands[] = {
	...
	{
		.name      = "get-sources",
		.help      = "Retrieves the list of sources which are observed by the subscription.\n"
		             "\n"
		             "options:\n"
		             "  SUBSCRIPTION:\t\tAn integer.\n"
		             "\n",
		.callback  = pk_shell_subscription_get_sources,
		.usage     = "subscription get-sources SUBSCRIPTION",
	},
	{
		.name      = "mute",
...
static EggLineCommand root_commands[] = {
	...
	},
	{
		...
		.usage     = "subscription [add-channel | add-source | get-buffer | get-created-at | get-sources | mute | remove-channel | remove-source | set-buffer | set-encoder | unmute]",
	},
	{
		.name      = "shell",
